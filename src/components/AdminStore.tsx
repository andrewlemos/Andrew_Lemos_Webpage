...
export const AdminStore = () => {
  ...
  const [editDigitalPdfUrl, setEditDigitalPdfUrl] = useState('');
  ...
  useEffect(() => {
    if (editingProduct) {
      ...
      setEditDigitalPdfUrl(editingProduct.digitalPdfUrl || '');
    }
  }, [editingProduct]);
  ...
  useEffect(() => {
    ...
    const unsubQuotes = onSnapshot(qQuotes, (snap) => {
      ...
      handleFirestoreError(error, OperationType.LIST, 'ecom_quotes');
    });
    ...
    const unsubCustomers = onSnapshot(collection(db, 'ecom_customers'), (snap) => {
      ...
      handleFirestoreError(error, OperationType.LIST, 'ecom_customers');
    });
  ...
  const handleCreateProduct = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const productPayload = {
        ...
        digitalPdfUrl: formData.digitalPdfUrl.trim(),
  ...
  const handleUpdateProduct = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const productPayload = {
        ...
        digitalPdfUrl: editDigitalPdfUrl.trim(),
  ...
  const handleRefundOrder = (orderId: string, refundValStr: string, notes: string) => {
    ...
    if (isNaN(value) || value <= 0) {
      alert("Por favor insira um valor válido de estorno.");
      return;
    }

    const currentRefunded = selectedOrder.refundedAmount || 0;
    const proposedTotalRefunded = currentRefunded + value;

    if (proposedTotalRefunded > selectedOrder.total) {
      alert(`O valor total de estornos acumulado (R$ ${proposedTotalRefunded.toFixed(2)}) não pode exceder o total geral pago pelo cliente (R$ ${selectedOrder.total.toFixed(2)}).`);
      return;
    }
            ...
            <div className="bg-white rounded-[2rem] w-full max-w-2xl max-h-[85vh] overflow-y-auto z-10 p-6 md:p-8 relative shadow-2xl text-xs font-sans">
              ...
              <div className="space-y-6">
                ...
                <div className="space-y-2 border-t pt-4">
                    ...
                    {selectedOrder.items.map((item, idx) => (
                      <div key={idx} className="flex items-center justify-between bg-[#FAFAFA] p-2.5 rounded-lg border gap-4">
                        ...
                        <div className="flex items-center gap-3 flex-shrink-0">
                          ...
                          <button
                            type="button"
                            onClick={() => handleDeleteOrderItem(selectedOrder.id!, idx)}
                  ...
                  {/* REFUNDS (ESTORNOS) SECTION */}
                  <div className="border-t pt-4 mt-2 space-y-3 bg-red-50/20 p-4 rounded-2xl border border-red-100/50">
                    <div className="flex items-center justify-between flex-wrap gap-2">
                      <span className="font-bold text-[10px] text-red-800 uppercase tracking-wider flex items-center gap-1.5">
                        <DollarSign className="w-4 h-4 text-red-600 font-bold" />
                        <span>Controle de Estorno de Pagamento</span>
                      </span>
                      {selectedOrder.refundStatus && selectedOrder.refundStatus !== 'none' && (
                        <span className={`px-2.5 py-0.5 rounded-md text-[9px] font-bold uppercase tracking-wider border ${
                          selectedOrder.refundStatus === 'total' 
                            ? 'bg-red-100 text-red-805 border-red-200' 
                            : 'bg-amber-100 text-amber-805 border-amber-200'
                        }`}>
                          Estorno {selectedOrder.refundStatus === 'total' ? 'Total' : 'Parcial'}
                        </span>
                      )}
                    </div>

                    <div className="text-[11px] text-gray-600 font-medium space-y-1">
                      <div>Total do Pedido: <span className="font-bold text-brand-ink">R$ {selectedOrder.total.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}</span></div>
                      <div>Valor já Estornado: <span className="font-bold text-red-700">R$ {(selectedOrder.refundedAmount || 0).toLocaleString('pt-BR', { minimumFractionDigits: 2 })}</span></div>
                      {selectedOrder.refundNotes && (
                        <div className="text-gray-500 text-[10.5px] bg-white p-2 border rounded-lg mt-1 italic">
                          <b>Motivo registrado:</b> "{selectedOrder.refundNotes}"
                        </div>
                      )}
                    </div>
                    ...
                    <form 
                      onSubmit={(e) => {
                        ...
                        const refundNotes = formData.get('refundNotes') as string;
                        handleRefundOrder(selectedOrder.id!, refundAmount, refundNotes);
                        e.currentTarget.reset();
                      }}
                      className="grid grid-cols-1 sm:grid-cols-3 gap-2.5 items-end pt-1"
                    >
                      <div>
                        <label className="text-[9px] font-bold text-gray-400 uppercase tracking-widest block mb-1">Valor do Estorno (R$)</label>
                        <input 
                          type="number" 
                          name="refundAmount"
                          step="0.01" 
                          ...
                          className="bg-white border border-gray-200/80 rounded-xl px-3 py-2 text-xs font-semibold w-full outline-none focus:ring-1 focus:ring-brand-wood"
                        />
                      </div>
                      <div className="sm:col-span-2 flex gap-1.5 items-end">
                        <div className="flex-grow">
                          <label className="text-[9px] font-bold text-gray-400 uppercase tracking-widest block mb-1 font-semibold">Justificativa / Observação</label>
                          <input 
                            type="text" 
                            name="refundNotes"
                            required
                            placeholder="Motivo (ex: Peça com imperfeição, desistência)" 
                            className="bg-white border border-gray-200/80 rounded-xl px-3 py-2 text-xs font-semibold w-full outline-none focus:ring-1 focus:ring-brand-wood"
                          />
                        </div>
                        <button 
                          type="submit"
                          className="bg-red-600 text-white px-4 py-2 h-9 rounded-xl font-bold hover:bg-red-700 text-[10px] transition-all cursor-pointer flex-shrink-0 flex items-center justify-center shadow-xs"
                        >
                          Efetuar Estorno
                        </button>
                      </div>
