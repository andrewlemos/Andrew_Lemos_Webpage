...
app.post("/api/vendas/webhook-melhorenvio", async (req, res) => {
  ...
  const signature = req.headers["x-me-signature"] || req.headers["X-ME-Signature"];
  ...
  if (secret && signature) {
    try {
      ...
      if (computedSignature !== signature) {
        console.warn(`[Webhook Melhor Envio] Assinatura inválida. Esperada: ${computedSignature}, Recebida: ${signature}`);
        const isProduction = process.env.NODE_ENV === "production" && process.env.MELHORENVIO_ENV !== "sandbox";
        if (isProduction) {
          return res.status(401).json({ error: "Assinatura X-ME-Signature inválida" });
        }
      } else {
        console.log("[Webhook Melhor Envio] Assinatura X-ME-Signature verificada com sucesso.");
      }
    } catch (sigErr) {
      console.error("[Webhook Melhor Envio] Erro ao verificar assinatura:", sigErr);
    }
  }
  ...
  try {
    ...
    // 2. Identify the order by shipment ID or fallback tags
    let orderDoc: any = null;
    const shipmentId = data.id;

    console.log(`[Webhook Melhor Envio] Buscando pedido associado à etiqueta ${shipmentId}...`);
    const snapshot = await adminDb.collection("ecom_orders")
    ...
    // Fallback: Check tags
    if (!orderDoc && data.tags && Array.isArray(data.tags)) {
      const orderIdTag = data.tags.find((t: any) => t.tag && typeof t.tag === "string");
      if (orderIdTag) {
        console.log(`[Webhook Melhor Envio] Etiqueta ${shipmentId} não localizada pelo ID no banco. Buscando por tag orderId: ${orderIdTag.tag}...`);
        const orderSnap = await adminDb.collection("ecom_orders").doc(orderIdTag.tag).get();
        if (orderSnap.exists) {
          orderDoc = orderSnap;
        }
      }
    ...
    if (!orderDoc) {
      console.warn(`[Webhook Melhor Envio] Pedido associado ao envio ${shipmentId} não foi localizado.`);
      // Return 200 to prevent Melhor Envio from retrying indefinitely
      return res.status(200).json({ status: "ignored", message: "Pedido não localizado." });
    }

    const orderId = orderDoc.id;
    const orderData = orderDoc.data();
    
    // Determine target statuses based on event and data status
    const meStatus = data.status || event.split(".")[1] || "released";
    ...
    if (data.tracking) {
      updatePayload.trackingCode = data.tracking;
      updatePayload.melhorEnvioTrackingCode = data.tracking;
    }

    if (data.tracking_url) {
      updatePayload.melhorEnvioTrackingUrl = data.tracking_url;
    }

    await adminDb.collection("ecom_orders").doc(orderId).update(updatePayload);
    console.log(`[Webhook Melhor Envio] Pedido ${orderId} atualizado com sucesso. Status Melhor Envio: ${meStatus} (${meStatusText}), Geral: ${overallStatus}`);
    ...
    if (overallStatus === "Entregue" && orderData.status !== "Entregue") {
      ...
      const baseUrl = `${protocol}://${host}`;
      sendDeliveredReviewRequestEmail(orderId, orderData, baseUrl).catch(err =>
        console.error("[Webhook Melhor Envio] Falha ao disparar e-mail de avaliação:", err)
      );
    }

    return res.json({ success: true, orderId, melhorEnvioStatus: meStatus, status: overallStatus });

  } catch (err: any) {
    console.error("[Webhook Melhor Envio] Erro ao processar atualização:", err);
    return res.status(500).json({ error: err.message });
  }
});
